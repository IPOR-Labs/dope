...
class OneMktStrategy:
  ...
  def act(self):
    ...
    agg = pd.concat(self.borrow_lend_data[pay_token], names=["datetime"]).unstack(level=0)
    whitelist = [self.pay_protocol] if self.pay_protocol else agg.apyBaseBorrow.columns
    borrow = agg.apyBaseBorrow[whitelist].min(axis=1)
    borrow_pool = agg.apyBaseBorrow.apply(pd.to_numeric, errors='coerce').idxmin(axis=1)



    agg = pd.concat(self.borrow_lend_data[rec_token], names=["datetime"]).unstack(level=0)
    whitelist = [self.rec_protocol] if self.pay_protocol else agg.apyBaseBorrow.columns
