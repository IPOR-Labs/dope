...
class Plotter:
    ...
    def plot_pools(self, borrow_lend_data, xlim=None, title_prefix=""):
        ...
        for token in borrow_lend_data.keys():
            ...
            plt.figure(figsize=(16, 4))
            ax = plt.gca()
            borrow_lend_data.to_block(token).apyBase.resample("3D").mean().plot(
            ...
            if xlim is not None:
                _ = plt.xlim(pd.Timestamp(xlim[0]), pd.Timestamp(xlim[1]))
            plt.title(
                f"{title_prefix}Pool Daily Yield. Token:{token} ({n_pools:,} pools)"
            )
    ...
    def plot_arb_routes(self, run, xlim=None):
        ...
        colors = {
            "aave-v3": plt.cm.Blues(
                np.linspace(0, 0.5, color_counter["aave-v3"])[::-1]
            ),
            "aave-v2": plt.cm.Reds(np.linspace(0, 0.5, color_counter["aave-v2"])[::-1]),
            "compound-v2": plt.cm.Greens(
                np.linspace(0, 0.5, color_counter["compound-v2"])[::-1]
            ),
        }
        ...
        colors = {
            "aave-v3": plt.cm.Blues(
                np.linspace(c_left, c_right, color_counter["aave-v3"] + 1)[::1]
            ),
            "aave-v2": plt.cm.Reds(
                np.linspace(c_left, c_right, color_counter["aave-v2"] + 1)[::1]
            ),
            "compound-v2": plt.cm.Greens(
                np.linspace(c_left, c_right, color_counter["compound-v2"] + 1)[::1]
            ),
        }
        ...
        for token in run.run_data.keys():
            ...
            for k, df in run.run_data[token].items():
                ...
                plt.plot(
                    df.apyBase,
                    label=_label,
                    color=colors[label][color_counter[label]],
                    alpha=0.3,
                )
                color_counter[label] -= 1

            best_triangle = (
                run.run_data.to_block("USDC").apyBase.mean().sort_values().index[-1]
            )
            plt.plot(
                run.run_data[token][best_triangle].apyBase,
                "--",
                color="black",
                label="Best Route",
            )
            _ = plt.legend(
                loc="upper left", bbox_to_anchor=(1, 1), title="Starting Protocol"
            )
        ...
        plt.show()
        r_t = (
            (run.run_data[token][best_triangle].apyBase / 100 / 365 + 1).cumprod() - 1
        ) * 100
        plt.plot(r_t)
