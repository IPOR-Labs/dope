...
class MktImpactLender(BaseAgent):
    ...
    def mktImpactOptimizer(self, data):
        ...
        slope = {
            key: self.engine.mkt_impact[key].get_slope(
                data[key].utilizationRate.iloc[-1]
            )
            for key in data.keys()
        }
        cplus = -np.ones(n) * [
            CAP / data[key].tvlUsd.resample("7D").mean()[-1] * slope[key]
            for key in data.keys()
        ]
        ...
        second_hiest_sigma_return = (
            mus[sorted_indices[0]] + mus[sorted_indices[0]]
        ) / 2
        B = opt.matrix(
            np.block(
                [
