...
class ConfidenceBandTrigger(BaseTrigger):
  ...
  def rates(self):
    ...
    sigma = self.est.rolling_fit_sigma(df=_df, lag=self.lagSD, rt_col=self.rt_col) # identify which column to use for calculating triggers
    if (self.equaliseSDs):
  ...
  def dates(self):
    ...
    for pair in self.pairs():
      ...
      t = t.fillna(0) # if NaN, the zero will mean t[diff_1] and t[diff_2] will neither be >0 nor <0 so no trigger should eventuate
