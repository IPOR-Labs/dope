...
class TokenPortfolio:
  ...
  def weights(self, price_row):
    ...
    allocation = {}
    allocation.update({protocol: capital/total for protocol, capital in self.deposit.allocation_value(price_row).items()})
    allocation.update({protocol: -capital/total for protocol, capital in self.debt.allocation_value(price_row).items()})
    allocation.update({protocol: capital/total for protocol, capital in self.cash.allocation_value(price_row).items()})
    return allocation
...
class LoopBacktester:
  ...
  def __init__(self, strategy, data, price_data, liquidation_threshold, mkt_impact=None):
  ...
  def health_factor(self, price_row):
    ...
    hf = self.π.deposit.capital(price_row) / self.π.debt.capital(price_row) * self.LT
    return hf
  ...
  def compound_account(self, account, rate_column, date_now, price_row):
    ...
    for mkt, capital in account.allocation.items():
      ...
      impacts[mkt] = self.mkt_impact[mkt].impact(date_now, value, is_borrow=is_borrow)
  ...
  def __call__(self, start_timestamp=None, end_timestamp=None):
    ...
    for i in range(0, len(self.dates[:])-1):
      ...
      r_breakdown_deposit, impact_deposit = self.compound_account(self.π.deposit, "apyBase", date_now, price_row=price_row)
      r_breakdown_debt, impact_debt = self.compound_account(self.π.debt, "apyBaseBorrow", date_now, price_row=price_row)
      ...
      if len(self.π.weights(price_row))>0:
        rows.append([date_now, self.π.weights(price_row), rate, health_factor, self.π.capital(price_row), r_breakdown, r_breakdown_deposit, r_breakdown_debt, impact_deposit, impact_debt])
    ...
    strategy = pd.DataFrame(rows, columns=["datetime", "ws", "rate", "health_factor", "capital", "r_breakdown", "r_breakdown_deposit", "r_breakdown_debt", "impact_deposit", "impact_debt"])
